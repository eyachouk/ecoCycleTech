...
public class CertificatRecyclageServiceImpl implements ICertificatRecyclageService {
    ...
    public void removeCertificatRecyclage(int idCertificatRecyclage) {
        System.out.println("deleting inside removeCertificatRecyclage");
    ...
    @Override
    public CertificatRecyclage findByDemandeId(int demandeId) {
        return certificatRecyclageRepository
                .findByDemandeRecyclage_IdDemandeRecyclage(demandeId)
                .orElseThrow(() -> new RuntimeException("No certificate for demande "+demandeId));
    }
